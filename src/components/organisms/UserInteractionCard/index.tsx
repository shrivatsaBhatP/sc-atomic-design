...
import { Status, Steps, Types } from '../../../common/enums';
...
const UserInteractionCard: React.FC = () => {
  ...
  const handleNextClick = () => {
    ...
    if (userData.status === Status.RESET) return;

    let newStep = userData.step + 1;
    newStep === 4 && userData.setValue(initialValue);
    newStep > 3 && (newStep = 1) && userData.setStatus(Status.RESET);
  ...
  const handleCancel = () => {
    ...
    userData.setStatus(Status.RESET);
  };
